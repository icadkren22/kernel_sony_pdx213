...
static int egis_proc_show(struct seq_file *m, void *v)
{
	unsigned char new_sn[3];
	new_sn[0] = 0x00;
	new_sn[1] = sn[6]==0x01 ? 0x37 : 0x33;
	new_sn[2] = sn[7]==0x03 ? 0x30 : 0x40;

	seq_printf(m, "%02x%02x%02x%02x%02x%02x%02x%02x\n", new_sn[0], new_sn[1], sn[1], sn[2], new_sn[2], sn[3],sn[4],sn[5]);
	return 0;
}
static int egis_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, egis_proc_show, NULL);
}
static const struct file_operations egis_proc_fops = {
	.open                = egis_proc_open,
	.read                = seq_read,
	.llseek              = seq_lseek,
...
int egisfp_probe(struct platform_device *pdev)
{
	...
	proc_create("egis_sn", 0644, NULL, &egis_proc_fops);
